 ...
 * repo. Catches the spec rule "no Postmark outbound, no Apollo/Clay/
 * LinkedIn integrations at MVP". Resend and Mailgun are approved outbound
 * providers (added post-MVP as no-credit-card alternatives to SES).
 */
...
describe('no provider source mentions a forbidden vendor as a sender', () => {
  it('no file exports an outbound provider tied to Postmark or scraper-only vendors', async () => {
